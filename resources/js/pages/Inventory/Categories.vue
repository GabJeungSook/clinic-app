...
defineOptions({ layout: { breadcrumbs: [{ title: 'Inventory', href: '/inventory' }, { title: 'Categories', href: '/inventory/categories' }] } });

const props = defineProps<{
    categories: {
        data: Array<{ id: string; name: string; items: number }>;
...
const submitSearch = () => router.get('/inventory/categories', { search: search.value }, { preserveState: true, replace: true });

const form = useForm({ name: '' });
const add = () => form.post('/inventory/categories', { preserveScroll: true, onSuccess: () => form.reset() });

const editingId = ref<string | null>(null);
const draft = ref('');
const startEdit = (c: { id: string; name: string }) => { editingId.value = c.id; draft.value = c.name; };
const saveEdit = (id: string) => router.put(`/inventory/categories/${id}`, { name: draft.value }, { preserveScroll: true, onSuccess: () => (editingId.value = null) });
const remove = (id: string) => router.delete(`/inventory/categories/${id}`, { preserveScroll: true });
</script>
            ...
            <CardContent class="p-0">
                <table class="w-full text-sm">
                    <thead class="border-b bg-muted/40 text-left text-muted-foreground">
                        <tr><th class="px-4 py-2 font-medium">Name</th><th class="px-4 py-2 text-center font-medium">Items</th><th class="px-4 py-2"></th></tr>
                    </thead>
                    <tbody>
                        <tr v-for="c in categories.data" :key="c.id" class="border-b last:border-0">
                            ...
                            <td class="px-4 py-2 text-center text-muted-foreground">{{ c.items }}</td>
                            <td class="px-4 py-2 text-right">
                                <Button v-if="editingId !== c.id" variant="ghost" size="icon-sm" @click="startEdit(c)"><Pencil class="size-4" /></Button>
                                <ConfirmDialog title="Delete category?" description="Items in this category will become uncategorised." @confirm="remove(c.id)">
